...
`spontaneous_frequency` 默认是 `0.15`。Planner 使用现有近期账本中的真实表情投递比例控制日常 `optional`，不新增模型调用或数据库查询；用户明确索要表情时不受该频率限制。该配置支持全局、群和用户作用域，可通过 `admin_set_config` 热修改。

`pool_capacity` 未设置表示无限；两个 cooldown 都允许 `0` 表示关闭。`storage_root` 和预览尺寸是启动配置。不存在 `emoji.review_enabled`。

确定性表情意图识别、真实回执校验和失败文字属于固定正确性契约，不能通过降低 Planner 超时或
放宽工具范围关闭。本次频率配置不需要数据库迁移。
