...
Plugin API v1 是 API 访问治理，不是 Python/OS 沙盒。插件与 Yuki 同进程运行，因此管理员必须把插件源码视为与 Bot 本身同等级的本地可信代码。恶意插件理论上可以使用 Python 能力绕过 SDK；1.6.0 不承诺安全运行未知第三方代码。
...
2. Planner、插件、历史、网页、OCR、记忆和用户自报都不能授予管理员权限。
3. Planner 与插件只能缩小后端能力。
