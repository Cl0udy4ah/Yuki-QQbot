...
稳定阶段顺序由 Host `PromptRegistry` 决定。核心安全片段优先受保护，插件片段位于 `plugin_context`/`tool_guidance`；同阶段按优先级和稳定 ID 排序。Planner 计划是 Host 可信约束，插件上下文不能修改它。

预算取以下限制的最小值：片段 `max_characters`、Manifest `limits.prompt_characters`、每片段/每插件/全插件 Host 配置。超过预算的低优先级片段会裁剪或拒绝，不能挤掉核心 Prompt。

## 目标

`PromptTarget` 包括 `planner`、`agent`、`both`、`plugin_session`。注册器和 Host 还会检查当前阶段是否允许该目标；不要把用户输入、网页正文或 OCR 伪装成可信 Fragment。
