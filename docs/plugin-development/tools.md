...
工具本地名必须匹配 `[a-z][a-z0-9_]{0,63}`。Host 生成全局唯一的模型工具名并处理过长名称，插件不要依赖该内部变换。
...
- Planner `tool_mode` 未设为 `none`，只读模式只保留 Host 认定的只读风险；
- 图片轮次、联网后撤权和自动化委托未将它移除。

Planner 不能添加工具。`ToolResult`/输出数据作为不可信工具结果传给模型，不能成为系统指令或权限凭证。
