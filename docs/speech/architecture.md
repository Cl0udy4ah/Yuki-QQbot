...
Yuki 1.8.0 的出站语音链路是 `Planner → SpeechService → Unix Domain Socket →
Genie-TTS Worker → 本地 GPT-SoVITS ONNX → WAV → OneBot record`。音频不经过 LLM，
也不调用任何云端 TTS。主 Bot 不依赖 Genie、ONNX Runtime 或 PyTorch。
