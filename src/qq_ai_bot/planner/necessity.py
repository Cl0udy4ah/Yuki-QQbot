"""Pure deterministic reply-necessity scoring for private and group messages."""
...
from qq_ai_bot.planner.models import PlannerSignal, ReplyNecessitySnapshot
...
class ReplyNecessityScorer:
    ...
    def score(self, features: ReplyNecessityFeatures) -> ReplyNecessitySnapshot:
        ...
        forced = (
            features.scope_type is ScopeType.PRIVATE
            or features.reply_target_is_bot
            or features.mentions_bot
        )
        should_enter = bool(text or features.media_only) and (forced or score >= self._threshold)
        return ReplyNecessitySnapshot(
            score=score,
            should_enter_planner=should_enter,
