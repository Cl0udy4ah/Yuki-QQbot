...
def _extract_segments(
    ...
    *,
    self_id: str,
) -> tuple[str, bool, tuple[MessageAttachment, ...]]:
    text_parts: list[str] = []
    mentions_bot = False
    ...
    for segment in segments:
        ...
        if segment_type == "text":
            ...
            if target == self_id:
                ...
                text_parts.append(f"[提及用户:{target}]")
    ...
    return sanitize_input("".join(text_parts)), mentions_bot, tuple(attachments)


def _reply_text(reply_message: Message | None, *, self_id: str) -> str | None:
    if reply_message is None:
        return None
    text, _, _ = _extract_segments(reply_message, self_id=self_id)
    return text or None
