...
class MemoryReleaseCheck:
    ...
    async def run(self, *, database_url: str | None = None) -> ReleaseCheckReport:
        ...
        items.append(self._item("alembic_head", head == "0025", f"Alembic head is {head}"))
        try:
    ...
    def _migration_contract_item(self) -> ReleaseCheckItem:
        ...
        return self._item(
            "migration_contract",
            not missing and self._alembic_head() == "0025",
            "fresh/upgrade matrix is current through 0025"
            if not missing
    ...
    def _plugin_contract_item(self) -> ReleaseCheckItem:
        ...
        return self._item(
            "plugin_api_1_0",
            not incompatible,
            f"{len(manifests)} manifests declare Plugin API 1.0 compatibility"
            if not incompatible
    ...
    async def _database_integrity(database: Database) -> bool:
        ...
        return integrity == "ok" and not foreign_keys and str(revision) == "0025"
