"""Focused contracts for Planner signals contributed by Plugin API v1."""
...
def _register(
    ...
    registry.registrar(
        plugin_id,
        (PluginPermission.PLANNER_SIGNAL_REGISTER,),
    ).register_planner_signal(PlannerSignalRegistration(name=name, provider=provider))
...
async def test_contextual_provider_receives_real_message_origin_and_scope() -> None:
    ...
    runtime = _runtime()
    received_contexts: list[PlannerSignalContext] = []
    scope_calls: list[tuple[str, InboundMessage, TurnOrigin, RuntimeConfigSnapshot]] = []

    async def provider(context: PlannerSignalContext) -> SdkPlannerSignal:
        received_contexts.append(context)
        return SdkPlannerSignal(
    ...
    adapter = PluginPlannerSignalAdapter(
        registry,
...
async def test_parameterless_provider_remains_backward_compatible() -> None:
    registry = ExtensionRegistry()
    calls = 0

    async def legacy_provider() -> SdkPlannerSignal:
        nonlocal calls
        calls += 1
        return SdkPlannerSignal(
            ...
            summary="Legacy callback still works",
            confidence=0.75,
        )

    _register(registry, "com.example.legacy", "legacy", legacy_provider)
    adapter = PluginPlannerSignalAdapter(registry)
...
async def test_provider_and_invocation_scope_failures_are_isolated() -> None:
    ...
    async def good_provider(_context: PlannerSignalContext) -> SdkPlannerSignal:
        return SdkPlannerSignal(
    ...
    async def failing_provider(_context: PlannerSignalContext) -> SdkPlannerSignal:
        raise RuntimeError("one plugin must not break planning")

    async def provider_behind_broken_scope(
        _context: PlannerSignalContext,
    ) -> SdkPlannerSignal:
